...
dictionary_list = []
mlresponse_list = []
file1 = open("output_Experiment5.txt", "w")  # write mode
...
#     training_data, validation_data = train_test_split(training_data, test_size=TEST_SIZE, random_state=rs)
    ...
    testing_data = bugs_openoffice_df.copy(deep=True)
...
#     print(dictionary_list)
    ...
    sorted_severe_lexicons = dict(sorted(severedictionary_list.items(), key=lambda item: item[1]["ratio"], reverse=True))
    sorted_nonsevere_lexicons = dict(sorted(nonseveredictionary_list.items(), key=lambda item: item[1]["ratio"], reverse=True))
